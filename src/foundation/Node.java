...
public class Node
{
    ...
    private double x;
    private double y;
    private final int id; // IMPORTANT! This id == id in the DB of the person which an instance of this class represents

    Node(final int id, final String firstName, final String lastName, final double x, final double y,
    ...
    {
        return (selected)? Node.SELECTED_NODE_COLOR : (male)? Node.MALE_NODE_COLOR : Node.FEMALE_NODE_COLOR;
    }
    ...
    private void drawNodeWithName(final @NotNull Graphics2D g2d, final @NotNull Camera camera)
    {
        ...
        if ( MainFrame.graphicsOptions.get(MainFrame.FILLED_CIRCLES) )
        {
            g2d.setColor( (male)? Node.MALE_FONT_COLOR : Node.FEMALE_FONT_COLOR );
        }
        else
        {
