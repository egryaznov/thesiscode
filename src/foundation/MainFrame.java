...
class MainFrame extends JFrame
{
    ...
    private @Nullable GenealogyView view;
    ...
    /*
        Initialize the genealogy view in which we will draw the family tree.
     */
    private void initView()
    {
        ...
        {
            view.stopAnimation();
            MainFrame.this.remove(view);
        }
        view = new GenealogyView();
        add( view );
        MainFrame.this.pack();
        this.setSize(DEFAULT_WIDTH, DEFAULT_HEIGHT);
        this.setLocationRelativeTo(null);
    ...
    private void commitNodesToDatabase()
    {
        if (view != null)
        {
            final @NotNull GenealogyModel model = view.getModel();
            for (final Node node : model.getNodes())
            {
                model.commitToDatabase(node);
            }
        }
    }
